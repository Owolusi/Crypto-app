...
function styleForms () {
	$(".simple-select").each (function () {
		...
		$sel.styler ({
			selectSearch:(! $sel.hasClass ("no_search")),
			onFormStyled:function () {
				var $select = $sel.next ();
				$sel.find (".simple-select .jq-selectbox__select").attr ("field_name", $sel.attr ("name"));
			}
		});
	});
/*
	$('.simple-select').styler({
		selectSearch:(! $(this).hasClass ("no_search")),
		onFormStyled:function () {
...
//cl (response);
				if (obj === undefined || obj === false || typeof obj != "object") {
					processAjaxSimple (JSON.parse (response), fa, fe);
				}
...
function processAjaxSimple (res) {
	...
	if (! isVoid (res.e)) {
		if (! isVoid (res.e.g)) {
			...
			if (! isVoid (t)) {
				openModal ("error", [t], "info");
//				ppError (t); /* ? */
			}
		}
		
		else if (! isVoid (res.e.i)) {
...
function processAjaxRequest (res, obj, fa, fe) {
	...
	if (res.status == "success") {
		...
		if (! isVoid (res.m.g)) {
			...
			if (! isVoid (t)) {
				if (res.close_other == 1) {
					closeOtherModals ();
				}
				
				setTimeout (function () {
					openModal ("message", [t], "info");
				}, 100);
			}
		}
	...
	if (! isVoid (res.e)) {
		...
		if (! isVoid (res.e.i)) {
			for (var k in res.e.i) {
				...
				if (! isVoid ($input) && typeof $input == "object") {
					$input.addClass ("ferror");
					var elId = Number (new Date ());
					$("<div class='input_message' element_id='"+ elId +"'>"+ res.e.i[k] +"</div>").insertAfter ($input).css ("opacity", 1);
				}
...
function tabCallBack ($this) {
	if (! isVoid ($this)) {
		...
		$(".tab.master[group='"+ tabGroup +"'][tab='"+ tabId+"']").addClass ("active");
...
function addCookie (name, value, days, sec) {
	var expires;
	days = parseInt (days);
		...
		if (days > 0) {
			if (! isVoid (name)) {
				if (! isVoid (value)) {
					...
					expires = "; expires=" + date.toGMTString() + sec;
				}
			}
		}
	}
...
function setSelectCountry (ctr) {
	if (isVoid (ctr)) {
		ctr = getCookie ("usr_ctr");
		$(".usr_ctr").val (ctr).change ();
	}
...
function toDec (sum, dec) {
	sum = Number (sum);
	dec = Number (dec);
	
	if (typeof sum == "number") {
		if (typeof dec == "number") {
			return Math.round (sum * Math.pow (10, dec)) / Math.pow (10, dec);
		}
		else {
			return sum;
		}
