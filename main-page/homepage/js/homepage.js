...
$(document).ready (function () {
	...
	if (! isVoid (hostName)) {
		$("#home_domain_name").html (hostName);
	}
//	paginate (1);
});
...
$(document).on ("click", "#view_next_reviews", function (e) {
	e.preventDefault ();
	
	ajax ("formaction=reviews&review=" + encodeURIComponent ($(this).attr ("review")));
}) 



function updateReviews (obj) {
	if (typeof obj == "object") {
		if (! isVoid (obj.code)) {
			$("#reviews_box_").html (obj.code);
			$("#view_next_reviews").attr ("review", obj.review);
		}
	}
}