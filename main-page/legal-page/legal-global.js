...
/*
$(window).resize (function () {
	cl ($(window).outerWidth () + " x " + $(window).outerHeight ());
})
*/
...
$(document).ready (function () {
    ...
    */
    ...
    //		var $offset = new Date();
        ...
        var ctr = getCookie ("usr_ctr");
        
        if (isVoid (ctr)) {
            ajax ("formaction=misc&action=usr_ctr");
        }
        else {
            setSelectCountry (ctr);
        }
        
        var ref = getGetParam ("ref").replace (/\+/g, ' ');
        ...
        if (! isVoid (ref)) {
            if (ref !== undefined && ref != "undefined") {
                addCookie ("ref", ref, 1);
                $("input[name='upline']").val (ref);
                refSet = true;
            }
        }
        
        if (refSet === false) {
            ref = decodeURI (getCookie ("ref"));
            
            if (! void (ref) && ref != "undefined" && ref !== undefined) {
                $("input[name='upline']").val (ref);
            }
        }
        ...
        if (langCode == "en" || langCode == "es" || langCode == "zh") {
            $(".select_language").val (langCode).trigger ("refresh");
        }
        else {
            $(".select_language").val ("en").trigger ("refresh");
        }
        
    //	cl ($("var[n='acc_rest']"));
        
        if ($("var[n='acc_rest']").length > 0) {
            openModal ("acc_rest");
        }
    });
    ...
    $(document).on ("input", ".ferror", function () {
        var $el = $(this);
        var $box = $el.parents (".field_box_");
        
        $box.removeClass ("ferror");
    ...
    function openModal (name, inputs, type, fz, fa) {
        ...
        if (! isVoid (name)) {
            ...
            if (! isVoid (inputs)) {
                query = "?inputs=" + encodeURIComponent (inputs);
            }
    ...
    */
                    $obj.find (".modal_content").load (path, function () {
                        $obj.addClass ("active");
                        cbinit ();
                        styleForms ();
    ...
    $(document).on ("input", ".home_calc #amount", function () {
        ...
        if (! isNaN (amount)) {
            if (! isVoid (amount)) {
                ...
                daily = Math.round (amount * 0.12 * 100) / 100;
                total = Math.round (daily * 12 * 100) / 100;
    ...
    $(document).on ("click", ".check_click", function (e) {
        ...
        if (e.originalEvent === undefined) {
            ...
            openModal ("check_click", [fName], 'info');
        }
    });
        ...
        if (! isVoid (fName)) {
            if (! isVoid ($form)) {
                ...
                setTimeout (function () {
                    ...
                    closeModal ("check_click");
                    ajax ($form.serialize (), $form.find (".check_click"));
